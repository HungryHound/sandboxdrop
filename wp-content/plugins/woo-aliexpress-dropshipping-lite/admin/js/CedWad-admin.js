(function( $ ) {
	...
	$(document).on( 'click', '.CedWad_new_orders', function(e){
		e.preventDefault();
		$( document ).find( '.CedWad_loader' ).show();
		var new_orders = {} ;
		var products_in_orders;
		// new_orders[$(this).attr( 'data-orderId' )] = $(this).attr( 'data-products-in-order' );
		var order_id = $(this).attr( 'data-orderId' );
		products_in_orders = $(this).attr( 'data-products-in-order' );
		$.ajax({
			url : CedWad_action_handler.ajax_url,
			type : 'post',
			data : {
				action : 'CedWad_ajaxFetchSingleOrderDetails',
				_nonce : 'CedWad_ajaxFetchSingleOrderDetails',
				order_id : order_id,
				products_in_orders : products_in_orders
			},
			success : function( response ) 
			{
				CedWad_get_order_fulfillment(response);
	...
	$( document ).on( 'click', '#CedWad_createOrderOnAliexpress', function(){
		$( document ).find( '.CedWad_loader' ).show();
		var new_orders = {} ;
		var products_in_orders;
		$( '.CedWad_new_orders' ).each( function(){
			new_orders[$(this).val()] = $(this).attr( 'data-products-in-order' );
		} );
		...
		$.ajax({
			...
			type : 'post',
			data : {
				action : 'CedWad_fetchOrderDetails',
				_nonce : 'CedWad_fetchOrderDetails',
				details : new_orders
			},
			success : function( response ) 
			{
				...
				CedWad_get_order_fulfillment(data);
